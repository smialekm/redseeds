...
import eu.redseeds.scl.impl.rsl.rsldomainelements.terms.ConditionalConjunctionImpl;
 ...
 */
public class Graph2OWLInstance {
	 ...
	 */
	public Graph2OWLInstance(Document doc, Graph g, boolean edgeClasses2Properties,
	...
	private void convertVertex(Vertex v, String vElemId, ProgressFunction pf) {
		...
		if (owlClass.startsWith("rsl.rsldomainelements.terms.")) {
			...
			if (owlClass.endsWith("Noun")) {
				...
				uid = "" + noun.getSynonymUid();
				rdfAbout = WordNet2OWL.NOUN_SYNONYM;
			} else if (owlClass.endsWith("Verb")) {
				VerbImpl verb = (VerbImpl)v;
				uid = "" + verb.getSynonymUid();
				rdfAbout = WordNet2OWL.VERB_SYNONYM;
			} else if (owlClass.endsWith("Determiner")) {
				DeterminerImpl det = (DeterminerImpl)v;
				uid = "" + det.getSynonymUid();
				rdfAbout = WordNet2OWL.DETERMINER_SYNONYM;
			} else if (owlClass.endsWith("ConditionalConjunction")) {
				ConditionalConjunctionImpl cond = (ConditionalConjunctionImpl)v;
				uid = "" + cond.getSynonymUid();
				rdfAbout = WordNet2OWL.CONDITIONAL_CONJUNCTION_SYNONYM;
			}
			if (uid != null) {
				Element wnLink = doc.createElement("rsl.rsldomainelements.terms.TermLinksToWordNetEntry");
				vElem.appendChild(wnLink);
				Element wnTerm = doc.createElement(rdfAbout);
				wnTerm.setAttribute("rdf:about", wn20schema + rdfAbout + "." + uid + "_instance");
