...
public class AddRequirementsPackageAction implements IWorkbenchWindowActionDelegate, IViewActionDelegate{
	...
	public void run(IAction action) {
			...
			if(tr.length > 0) {
				if(tr[0] != null) {
					...
					if (!(parent instanceof RequirementsPackageDTO 
							|| parent instanceof RequirementsSpecificationDTO)) {
						showWarning();
						return;
					}
				}
		...
		SCLElementWizard wizard = new SCLElementWizard(RequirementsPackageDTO.class);
	...
	private void showWarning() {
		...
		warnMB.setMessage("You can create a Requirements Package only as a Requirements Package or a Requirements Specification child");
