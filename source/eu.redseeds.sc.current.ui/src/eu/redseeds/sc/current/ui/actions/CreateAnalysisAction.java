...
public class CreateAnalysisAction implements IWorkbenchWindowActionDelegate,
IViewActionDelegate {
...
//		IConversions eaConversions = Activator.getDefault().getIConversionsInstance();	
		...
		TransformationProfile tmpRtoKey = null;
		...
		for (TransformationProfile prof : tmpTransfList) {
			if (prof.getType().getType().equals("KEYWORD"))
				tmpRtoKey = prof; 
			if (prof.getType().getType().equals("REQ_ANALYSIS_TRANSFORMATION"))
				tmpRtoA = prof; 
			if (prof.getType().getType().equals("UML_EA_TRANSFORMATION"))
				tmpUMLtoEA = prof; 
		}
		
		if (tmpRtoA == null ||  tmpUMLtoEA == null || tmpRtoKey == null) {
...
//		ITreeSelection currSelection = (ITreeSelection) SCProjectHelper.getSelection(); - defined above
	    ...
	    molaProperties.setProperty("uml_to_ea_kind", "Analysis");
	    try {
		...
		workbench.getDisplay().syncExec(new Runnable() { 
			public void run() { 
				...
				if (window != null) { 
					...
					RunTransformationsJob action = new RunTransformationsJob("Generating Analysis model",
							                                                  exTransfList,
							                                                  currSCProject,
							                                                  true,
							                                                  false,
					...
					try {
			            ...
			            Activator.log("Error during generating Analysis", Status.ERROR);
			        }
				}
			}
		});
		...
		Activator.log("Analysis generation completed ...", Status.INFO);
		MessageBox confirmMB = new MessageBox(SCProjectHelper.getShell(), SWT.ICON_INFORMATION);
		confirmMB.setMessage("Analysis generation completed!");
		confirmMB.open();
