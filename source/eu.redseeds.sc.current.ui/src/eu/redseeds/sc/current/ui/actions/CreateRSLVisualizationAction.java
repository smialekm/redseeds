...
public class CreateRSLVisualizationAction implements IWorkbenchWindowActionDelegate,
IViewActionDelegate {
...
//		IConversions eaConversions = Activator.getDefault().getIConversionsInstance();
		...
		TransformationProfile tmpRtoActivity = null;
		...
		for (TransformationProfile prof : tmpTransfList) {
			if (prof.getType().getType().equals("REQ_ACTIVITY_TRANSFORMATION"))
				tmpRtoActivity = prof;
			if (prof.getType().getType().equals("UML_EA_TRANSFORMATION"))
				tmpUMLtoEA = prof;
		}

		if (tmpRtoActivity == null ||  tmpUMLtoEA == null) {
	    ...
	    molaProperties.setProperty("uml_to_ea_kind", "RSL_Visualization");
	    try {
		...
		exTransfList.add(tmpRtoActivity);
		...
		workbench.getDisplay().syncExec(new Runnable() {
			public void run() {
				...
				if (window != null) {
					...
					RunTransformationsJob action = new RunTransformationsJob("Generating Requirements visualization",
							                                                  exTransfList,
							                                                  currSCProject,
							                                                  true,
							                                                  true,
					...
					try {
			            ...
			            Activator.log("Error during generating Requirements visualization: "+ex.getCause().getMessage(), IStatus.ERROR);
			        }
				}
			}
		});

		Activator.log("Requirements visualization generation completed ...", Status.INFO);
		MessageBox confirmMB = new MessageBox(SCProjectHelper.getShell(), SWT.ICON_INFORMATION);
		confirmMB.setMessage("Requirements visualization generation completed!");
		confirmMB.open();
	}
