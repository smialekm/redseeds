...
public class PreconditionSentenceFix extends ScenarioSentenceFix {
	...
	public void run(IMarker marker) {
		try {
			...
			if (marker.getResource() instanceof IProject) {
				...
				// get the sentence
				PreconditionSentenceDTO preCondSent = scProject
						.getBusinessLayerFacade()
						.getPreconditionSentenceByVertexID(
								elementID.intValue());
				if (preCondSent != null) {
					if (preCondSent.getOwningScenarios() != null) {
						if (preCondSent.getOwningScenarios().size() > 0) {
							scenario = preCondSent.getOwningScenarios().get(0);
						}
					}
				}
				if (scenario != null) {
