...
public class ProcessSensesAutoAddAndAssignmentAction extends AbstractProcessSensesAssignmentAction {
	
	public ProcessSensesAutoAddAndAssignmentAction(PhraseDTO phrase) {
		super(phrase);
	}
	
	public ProcessSensesAutoAddAndAssignmentAction(TermDTO term) {
		super(term);
	}
		...
		if (phrase == null){
			...
			if (phrase instanceof NounPhraseDTO){
				...
				for (TermDTO tmpTerm : tmpTerms){
					try {
						tmpTerm.autoAddAndAssignSense();
					} catch (NullPointerException e){
						e.printStackTrace();
					}
					if (tmpTerm.getSynonymUid() == 0 || null==RemoteJGWNL.getInstance().getTermSenseDTO(tmpTerm.getSynonymUid())){
