...
import eu.redseeds.scl.impl.rsl.rsldomainelements.terms.ModifierImpl;
...
public class ModifierDTOImpl extends ModifierImpl implements ModifierDTO {

	public ModifierDTOImpl(int arg0, Graph arg1) {
		super(arg0, arg1);
	...
	public boolean autoAssignSense() {
		TermSenseDTO[] senses = RemoteJGWNL.getInstance().getTermSenses(this.name.toLowerCase(), Constants.TERM_ADJECTIVE);
		if (senses.length > 0){
	...
	public void autoAddAndAssignSense() {
		if (!autoAssignSense()){
			this.setSynonymUid(RemoteJGWNL.getInstance().addNewSense(name, Constants.DEFAULT_GLOSS+" "+name, Constants.TERM_ADJECTIVE, true));
