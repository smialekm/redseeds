...
public class AddTestsPackageAction implements IWorkbenchWindowActionDelegate, IViewActionDelegate{
	...
	public void run(IAction action) {
			...
			if(tr.length > 0) {
				if(tr[0] != null) {
					...
					if (!(parent instanceof TestPackage 
							|| parent instanceof TestSpecification)) {
						showWarning();
						return;
					}
				}
		...
		SCLElementWizard wizard = new SCLElementWizard(TestPackage.class);
	...
	private void showWarning() {
		...
		warnMB.setMessage("You can create a Tests Package only as a Tests Package or a Test Specification child");
