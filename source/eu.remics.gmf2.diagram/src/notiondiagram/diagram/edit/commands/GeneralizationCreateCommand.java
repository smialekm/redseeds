...
public class GeneralizationCreateCommand extends EditElementCommand {
	...
	public boolean canExecute() {
		...
		return NotionDiagramBaseItemSemanticEditPolicy.getLinkConstraints()
				.canCreateGeneralization_4001(getContainer(), getSource(),
						getTarget());
	}
	...
	public boolean canRedo() {
		...
		if (null != getContainer().eResource()
				&& null == rmh
						.getSpecialisationByVertexID(((Generalization) ((CreateElementRequest) getRequest())
								.getNewElement()).getId())) {
			return false;
		}
	...
	protected CommandResult doExecuteWithResult(IProgressMonitor monitor,
		...
		Generalization newElement = NotiondiagramFactory.eINSTANCE
		...
		RecoveryModelHelper rmh = RecoveryModelHelper.instance(getContainer()
				.eResource());
		NotionSpecialisationDTO spec = rmh.getBussinessLayerFacade()
				.createNotionSpecialisationDTO();
		spec.setSpecialisedNotion(rmh.getNotionByVertexID(getSource().getId()));
		spec.setGeneralNotion(rmh.getNotionByVertexID(getTarget().getId()));
		newElement.setId(((NotionSpecialisation) spec).getId());

		Stereotype stereotype = rmh.getBussinessLayerFacade()
				.createSclkernel$Stereotype();
		stereotype.setName("temporary");
		((NotionSpecialisation) spec).addStereotype(stereotype);
